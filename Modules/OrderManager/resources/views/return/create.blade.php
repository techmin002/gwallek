<!-- ✅ Create Return Modal -->
<div class="modal fade" id="createReturnModal" tabindex="-1" role="dialog" aria-labelledby="createReturnLabel"
    aria-hidden="true">
                ...
                <div class="modal-body modal-body-advanced">
                    <div class="row g-3">
                        ...
                        <div class="col-md-12 mt-2">
                            ...
                            <textarea name="remarks" class="form-control summernote"></textarea>
                        </div>
                    </div>
                    ...
                    <button type="button" class="btn btn-outline-primary mt-3" id="addReturnProductRow">
...
<script>
    $(function() {
        ...
        const productsCreate = @json($products);

        function createReturnRow(index) {
            let options = productsCreate.map(p => `<option value="${p.id}">${p.name}</option>`).join('');
            return `
        ...
        $('#createReturnModal').on('shown.bs.modal', function() {
            returnRowIndex = 0;
            const container = $("#returnProductContainer");
            container.html('');
            container.append(createReturnRow(returnRowIndex++));
        });

        // Add row
        $("#addReturnProductRow").on("click", function() {
            $("#returnProductContainer").append(createReturnRow(returnRowIndex++));
        });
