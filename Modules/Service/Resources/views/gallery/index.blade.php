                        ...
                        <div class="card">
                            <div class="card-header">
                                @can('create_gallery')
                                    ...
                                    @include('service::gallery.create')
                                @endcan
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
                                        ...
                                        @foreach ($galleries as $key => $value)
                                            <tr>
                                                ...
                                                <td class="text-center"><img
                                                        src="{{ asset('upload/images/galleries/' . $value->image) }}"
                                                        width="120px" alt="{{ $value->name }}"> </td>
                                                <td class="text-center">
                                                    @if ($value->status == 'on')
                                                        <a href="{{ route('galleries.status', $value->id) }}"
                                ...
                                ">
                                                            ...
                                                            <form id="destroy{{ $value->id }}" class="d-none"
                                                                action="{{ route('galleries.destroy', $value->id) }}"
                                                                method="POST">
                                                                @csrf
                                                                @method('delete')
                                                            </form>
